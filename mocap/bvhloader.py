...
class BVHParser:
    ...
    def plot_joint_trajectory(self, joint_name):
        if joint_name not in self.joint_hierarchy and joint_name != self.root_joint:
        ...
        for frame in range(self.frames):
            ...
            if joint_name in positions:
                joint_pos = positions[joint_name]
                x_positions.append(joint_pos[0])
                y_positions.append(joint_pos[1])
                z_positions.append(joint_pos[2])

        fig, ax = plt.subplots(3, 1, figsize=(10, 8))
        fig.suptitle(f"Trajectory of Joint '{joint_name}'")

        ax[0].plot(x_positions, label='X Position')
        ax[0].set_ylabel('X Position')
        ax[0].legend()

        ax[1].plot(y_positions, label='Y Position')
        ax[1].set_ylabel('Y Position')
        ax[1].legend()

        ax[2].plot(z_positions, label='Z Position')
    ...
    def plot_all_joint_trajectories(self):
        joint_positions = {joint: {'x': [], 'y': [], 'z': []} for joint in self.joint_hierarchy}
        joint_positions[self.root_joint] = {'x': [], 'y': [], 'z': []}

        for frame in range(self.frames):
            positions = self.get_joint_positions(frame)
            for joint, pos in positions.items():
                joint_positions[joint]['x'].append(pos[0])
                joint_positions[joint]['y'].append(pos[1])
                joint_positions[joint]['z'].append(pos[2])

        fig, ax = plt.subplots(3, 1, figsize=(10, 8))
        fig.suptitle("Trajectories of All Joints")

        for joint, pos in joint_positions.items():
            ax[0].plot(pos['x'], label=f'{joint} X')
            ax[1].plot(pos['y'], label=f'{joint} Y')
            ax[2].plot(pos['z'], label=f'{joint} Z')
    ...
    def plot_leg_joint_trajectories(self):
        leg_joints = ["rButtock", "rThigh", "rShin", "rFoot", "lButtock", "lThigh", "lShin", "lFoot"]
        joint_positions = {joint: {'x': [], 'y': [], 'z': []} for joint in leg_joints}
        ...
        fig, ax = plt.subplots(3, 1, figsize=(10, 8))
        fig.suptitle("Trajectories of Leg Joints")
    ...
    def animate_leg_joints(self):
        ...
        def update(frame):
            ...
            positions = self.get_joint_positions(frame)
            def draw_bone(joint):
                if joint in positions and joint in leg_joints:
                    joint_pos = positions[joint]
                    for child in self.joint_hierarchy.get(joint, []):
                        if child in positions and child in leg_joints:
                            child_pos = positions[child]
                            ax.plot([joint_pos[0], child_pos[0]],
                                    [joint_pos[1], child_pos[1]],
                                    [joint_pos[2], child_pos[2]], 'bo-')
...
# 使用例
bvh_parser = BVHParser("bvh/49_01.bvh")  # BVHファイルを指定
#bvh_parser.animate()  # アニメーションを作成
bvh_parser.animate_leg_joints()
#bvh_parser.plot_all_joint_trajectories()  # すべての関節の軌道をプロット
#bvh_parser.plot_leg_joint_trajectories() 
mocap = Bvh(f.read())
print(mocap.joint_parent('mixamorig:Head').name)