...
try:
    ...
    while True:
        ...
        if pattern_index == 0:
            ...
            # Pattern 2: Sin wave
            odrv0_velocity = 8.0 * math.sin(2 * math.pi * 1.0 * elapsed_time)
        elif pattern_index == 2:
            # Pattern 3: Step function (alternating velocities)
            odrv0_velocity = 7.0 if int(pattern_time * 2) % 2 == 0 else -7.0
        elif pattern_index == 3:
            # Pattern 4: Ramp (linear increase then decrease)
            if pattern_time < pattern_duration / 2:
                odrv0_velocity = 10.0 * (pattern_time / (pattern_duration / 2))
            else:
                odrv0_velocity = 10.0 * (1 - (pattern_time - pattern_duration / 2) / (pattern_duration / 2))
        elif pattern_index == 4:
            # Pattern 5: Cosine wave with different frequency
            odrv0_velocity = 6.0 * math.cos(2 * math.pi * 0.5 * elapsed_time)
        else:  # pattern_index == 5
            # Pattern 6: Combined sin and cos waves
            odrv0_velocity = 4.0 * math.sin(2 * math.pi * 1.2 * elapsed_time) + 3.0 * math.cos(2 * math.pi * 0.8 * elapsed_time)
